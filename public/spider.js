...
var Bug = {
    ...
    makeBug: function() {
        if (!this.bug && this.active) {
            ...
            if (this.options.imageSprite == 'spider-sprite.webp')
    ...
    flyIn: function() {
        ...
        var side = Math.round(Math.random() * 4 - 0.5)
          ...
          , windowX = innerWidth || e.clientWidth || g.clientWidth
          , windowY = innerHeight || e.clientHeight || g.clientHeight;
        if (side > 3)
            side = 3;
        if (side < 0)
            side = 0;
    ...
    drop: function(deathType) {
        var startPos = this.bug.top
          ...
          , e = d.documentElement
          , g = d.getElementsByTagName('body')[0]
          , finalPos = innerHeight || e.clientHeight || g.clientHeight
    ...
    dropping: function(t, startPos, finalPos, rotationRate, deathType) {
        ...
        if (newPos >= finalPos) {
            ...
            this.bug.style.bottom = Math.ceil((rotationOffset - zoomOffset)) + 'px';
